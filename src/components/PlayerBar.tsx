        ...
        return (
          <div key={p.id} style={{
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              <ResIcon label="🍞" v={p.resources.food} />
              <ResIcon label="🍷" v={p.resources.wine} />
              <ResIcon label="☕" v={p.resources.coffee} />
              <ResIcon label="🍰" v={p.resources.cake} />
              <ResIcon label="💰" v={p.resources.money} />
            </div>
            <div style={{ fontSize: 11, color: '#666', marginTop: 4 }}>
              <span>客人{p.guestWaitingArea.length} | </span>
              <span>已招待{p.guestServedArea.length} | </span>
              <span>房间{p.builtRooms.length} | </span>
              <span>员工{p.staffCards.length}</span>
            </div>
          </div>
        )
      })}
...
function ResIcon({ label, v }: { label: string; v: number }) {
